...
### Milestone 0
- T0.1 Rust workspace (`crates/devpulse-core`, `crates/devpulse-discovery`,
  `crates/devpulse-cli`, `fixtures/`)
- T0.2 Process snapshot spike (`devpulse scan-processes`)
- T0.3 Socket ownership spike (`devpulse scan-sockets`)
- T0.4 TCP fixtures (`fixture-tcp-server`, `fixture-tcp-client`)
- T0.5 Project root resolver (`devpulse resolve-project`, `devpulse scan-projects`)
...
### Milestone 2
- T2.1 Snapshot loop (`SnapshotLoop` in `devpulse-server`)
...
Commands:
  ...
  cargo run --release -p devpulse-cli -- scan-processes --filter /private/tmp/devpulse-spike
  cargo run --release -p devpulse-cli -- scan-sockets --port 41011
  cargo run --release -p devpulse-cli -- scan-projects
  cargo run --release -p devpulse-cli -- bench --iterations 30
Tests: 47 passed, 0 failed
  - devpulse-core        26 unit (project resolver, redaction)
  - devpulse-discovery    9 unit (process, socket, platform)
  - devpulse-fixtures     3 unit + 6 integration (I1 port ownership, I2 local
                          topology, listener has no peer, fixture metadata,
                          end-to-end secret redaction, collector budget)
  - devpulse-cli          3 unit (rendering)
Notes:
  ...
  - Three-runtime fixture project at /private/tmp/devpulse-spike (Node 41010,
...
Commands:
  cargo fmt --all --check
  cargo clippy --workspace --all-targets -- -D warnings
  cargo test -p devpulse-server
Tests: 13 passed, 0 failed
  - devpulse-server 13 unit (snapshot loop, dto, security)
Notes:
  - Implemented SnapshotLoop in devpulse-server with continuous process/socket collection
...
`devpulse_discovery::platform::capabilities()` but are unverified.
