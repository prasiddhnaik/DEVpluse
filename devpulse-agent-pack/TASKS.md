...
```text
crates/devpulse-cli
crates/devpulse-core
crates/devpulse-discovery
fixtures/
```
...
```bash
cargo run -p devpulse-cli -- scan-processes
```
...
```bash
cargo run -p devpulse-cli -- scan-sockets
```
...
- DevPulse identifies the correct PID
- start fixture client
- DevPulse sees client/server connection if OS APIs permit
