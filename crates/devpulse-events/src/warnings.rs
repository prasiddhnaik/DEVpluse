...
//! to trust (`DECISIONS.md` D008). A rule fires from observations DevPulse
...
use devpulse_core::model::{
    DevPulseEvent, EventKind, Health, ResourceSample, Service, Severity, Warning,
};
...
impl WarningEngine {
    ...
    fn restart_loops(&self, input: &WarningInput<'_>, at: SystemTime) -> Vec<Warning> {
        let mut by_service: BTreeMap<&ServiceId, Vec<&DevPulseEvent>> = BTreeMap::new();
    ...
    fn health_failures(&self, input: &WarningInput<'_>, at: SystemTime) -> Vec<Warning> {
        ...
        input
            ...
            .map(|service| {
                let related: Vec<&DevPulseEvent> = recent
                    .iter()
                    .copied()
                    .filter(|event| {
                        matches!(
...
fn ids(events: &[&DevPulseEvent]) -> Vec<EventId> {
...
mod tests {
    ...
    use devpulse_core::model::{Endpoint, Protocol, ServiceKind};
    ...
    fn service(name: &str, port: Option<u16>) -> Service {
        Service {
            ...
            instances: vec![devpulse_core::model::ProcessInstance {
                pid: 100,
                parent_pid: None,
                executable: None,
                command: Vec::new(),
    ...
    fn restart(service: &Service, secs: u64, sequence: u32) -> DevPulseEvent {
        DevPulseEvent {
    ...
    fn samples(values: &[(f32, u64)]) -> Vec<ResourceSample> {
        values
            .iter()
            .enumerate()
            .map(|(i, (cpu, memory))| ResourceSample {
                at: at(i as u64),
                cpu_percent: *cpu,
                memory_bytes: *memory,
            })
