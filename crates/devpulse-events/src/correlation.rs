...
pub enum Relation {
    ...
    /// the closest thing DevPulse has to an explanation, and it is still only
    /// an ordering.
    PrecedingFileChange,
...
pub fn context(events: &[DevPulseEvent], anchor: &DevPulseEvent, window: Duration) -> EventContext {
...
pub fn preceding_file_change<'a>(
    events: &'a [DevPulseEvent],
    restart: &DevPulseEvent,
    within: Duration,
) -> Option<&'a DevPulseEvent> {
...
fn relation_between(anchor: &DevPulseEvent, other: &DevPulseEvent, offset_ms: i64) -> Relation {
...
fn project_of(event: &DevPulseEvent) -> Option<&ProjectId> {
    match &event.kind {
...
pub fn service_of(event: &DevPulseEvent) -> Option<&ServiceId> {
    match &event.kind {
...
mod tests {
    use super::*;
    use devpulse_core::model::Health;
    ...
    fn event(secs: u64, sequence: u32, kind: EventKind) -> DevPulseEvent {
        DevPulseEvent {
    ...
    fn restarted(secs: u64, sequence: u32) -> DevPulseEvent {
        event(
            secs,
            sequence,
    ...
    fn health_failed(secs: u64, sequence: u32) -> DevPulseEvent {
        event(
            secs,
            sequence,
