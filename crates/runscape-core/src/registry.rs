...
pub struct ServiceObservation {
    ...
    /// Docker stats when collected. Host processes leave this `None`.
...
impl ServiceRegistry {
    ...
    ///
    /// `observations` is the *complete* set of running processes Runscape could
    ...
    pub fn apply(
        ...
        for (id, entered) in &mut self.services {
            ...
            entered.service.measured_cpu_percent = None;
            entered.service.measured_memory_bytes = None;
    ...
    fn update(
        ...
        service.measured_cpu_percent = merged.measured_cpu_percent;
        service.measured_memory_bytes = merged.measured_memory_bytes;
...
struct Merged {
    ...
    measured_cpu_percent: Option<f32>,
    measured_memory_bytes: Option<u64>,
}
...
impl Merged {
    fn new(observation: ServiceObservation) -> Self {
        Self {
            ...
            measured_cpu_percent: observation.measured_cpu_percent,
            measured_memory_bytes: observation.measured_memory_bytes,
        }
    }
    ...
    fn absorb(&mut self, observation: ServiceObservation) {
        ...
        if observation.measured_cpu_percent.is_some() {
            self.measured_cpu_percent = observation.measured_cpu_percent;
        }
        if observation.measured_memory_bytes.is_some() {
            self.measured_memory_bytes = observation.measured_memory_bytes;
        }
    }
