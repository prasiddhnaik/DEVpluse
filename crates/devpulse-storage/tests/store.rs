...
use devpulse_core::model::{
    DevPulseEvent, Endpoint, EventKind, Health, ProcessInstance, Project, Protocol, ResourceSample,
    Service, ServiceKind, Severity, Warning,
};
use devpulse_core::project::{ProjectEvidence, ProjectMarker, RootKind};
use devpulse_core::{
    ConnectionId, ContainerIdentity, EventId, ProjectId, Runtime, ServiceFingerprint, ServiceId,
};
use devpulse_storage::{RetentionPolicy, Store};
...
fn one_of_every_event_kind(project: &ProjectId, service: &ServiceId) -> Vec<DevPulseEvent> {
...
fn opening_an_existing_database_is_idempotent_and_preserves_data() {
    ...
    let path = dir.path().join("state/devpulse.db");
    let project = project("/tmp/shop", "shop");
...
fn a_second_handle_on_the_same_file_sees_committed_rows() {
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("devpulse.db");
