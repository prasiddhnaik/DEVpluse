...
use runscape_core::model::{
    EventKind, Health, Project, ResourceSample, RunscapeEvent, Service, ServiceKind, Severity,
    Warning,
};
use runscape_core::{EventId, ProjectId, ServiceId};
...
impl Store {
    ...
    pub fn record_events(&self, events: &[RunscapeEvent]) -> Result<usize, StorageError> {
        if events.is_empty() {
            return Ok(0);
        }
    ...
    pub fn record_resource_sample(
        ...
        self.conn().execute(
            "INSERT INTO resource_samples (service_id, at, cpu_percent, memory_bytes,
                 virtual_memory_bytes, thread_count, disk_read_bytes, disk_write_bytes,
                 connection_count)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)",
            params![
                service.as_str(),
                ...
                bytes_to_sql(sample.virtual_memory_bytes),
                i64::from(sample.thread_count),
                bytes_to_sql(sample.disk_read_bytes),
                bytes_to_sql(sample.disk_write_bytes),
                i64::from(sample.connection_count),
            ],
        )?;
        Ok(())
    }
    ...
    pub fn recent_events(
        ...
        limit: usize,
    ) -> Result<Vec<RunscapeEvent>, StorageError> {
        let conn = self.conn();
...
fn event_from_row(row: &Row<'_>) -> Result<RunscapeEvent, StorageError> {
    Ok(RunscapeEvent {
...
/// API because the daemon's *live* ring buffer lives in `runscape-core`; this is
...
impl Store {
    ...
    pub fn resource_samples(
        ...
        let mut stmt = conn.prepare_cached(
            "SELECT at, cpu_percent, memory_bytes, virtual_memory_bytes, thread_count,
                    disk_read_bytes, disk_write_bytes, connection_count FROM resource_samples \
        ...
        let mut samples = collect(&mut stmt, (service.as_str(), count_to_sql(limit)), |row| {
            Ok(ResourceSample {
                ...
                virtual_memory_bytes: bytes_from_sql(row.get("virtual_memory_bytes")?),
                thread_count: u32::try_from(row.get::<_, i64>("thread_count")?).unwrap_or(0),
                disk_read_bytes: bytes_from_sql(row.get("disk_read_bytes")?),
                disk_write_bytes: bytes_from_sql(row.get("disk_write_bytes")?),
                connection_count: u32::try_from(row.get::<_, i64>("connection_count")?)
                    .unwrap_or(0),
            })
        })?;
        samples.reverse();
