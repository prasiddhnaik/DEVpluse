...
//! database written by a newer Runscape must not be silently downgraded, and a
//! path that cannot hold a database must be reported before the daemon starts
//! recording into nowhere.

use std::path::PathBuf;

/// Anything that can go wrong while persisting or reading Runscape state.
...
pub enum StorageError {
    ...
    /// The file was written by a newer Runscape. Opening it anyway would mean
    ...
    #[error(
        "database schema version {found} is newer than the supported version {supported}; \
         upgrade Runscape or remove the database file"
    )]
