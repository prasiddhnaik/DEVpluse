...
use devpulse_core::model::{DevPulseEvent, Project, ResourceSample, Service, Warning};
use devpulse_storage::{RetentionPolicy, StorageError, Store};
use tokio::sync::mpsc;
...
/// Default database location: `~/.devpulse/devpulse.db`.
///
...
pub fn default_database_path() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(home).join(".devpulse").join("devpulse.db"))
}
...
mod tests {
    ...
    fn event(secs: u64) -> DevPulseEvent {
        DevPulseEvent {
    ...
    fn the_default_path_lives_under_home() {
        ...
        assert!(path.ends_with(".devpulse/devpulse.db"), "{path:?}");
    }
}
