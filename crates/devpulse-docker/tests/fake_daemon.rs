...
//!
//! Docker is not installed everywhere DevPulse is developed, and a test that
...
use devpulse_docker::{ContainerCollector, ContainerState, DockerAvailability};
...
async fn assert_stats_off(collector: &devpulse_docker::BollardCollector, socket: &Path) {
    ...
    assert_eq!(service.health, devpulse_core::Health::Healthy);
    assert_eq!(
    ...
    assert_eq!(cache.health, devpulse_core::Health::Stopped);
...
async fn assert_stats_on(collector: &devpulse_docker::BollardCollector) {
