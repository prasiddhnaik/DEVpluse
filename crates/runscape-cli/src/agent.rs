...
use runscape_server::state::AppState;

/// Default events included in `runscape now`. Small enough for a prompt.
...
pub fn ready_line(addr: SocketAddr) -> Value {
    json!({
        ...
        "ui": format!("http://localhost:{}", addr.port()),
        "now": format!("runscape now --port {}", addr.port()),
    })
}
...
pub fn print_unreachable(addr: SocketAddr, err: impl std::fmt::Display) {
    let value = json!({
        "ok": false,
        "error": format!("{err}"),
        "hint": format!("runscape serve --headless --port {}", addr.port()),
    });
...
mod tests {
    ...
    fn here_drops_projects_that_do_not_enclose_cwd() {
        let filtered = filter_projects_here(
            json!([{"id": "x", "root": "/no/such/runscape-project-root"}]),
            Path::new("/tmp"),
        );
