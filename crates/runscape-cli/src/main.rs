//! Runscape discovery spike CLI (Milestone 0).
//!
...
use runscape_cli::agent::{self, NOW_EVENT_LIMIT};
use runscape_core::{NoProject, ProjectMatch, ProjectResolver, ResolverConfig};
...
use runscape_server::daemon::{Daemon, DaemonConfig};
use runscape_server::security::DEFAULT_PORT;

#[derive(Debug, Parser)]
#[command(
    name = "runscape",
    version,
    about = "Local-first observer. `runscape serve` opens the visual dashboard; `runscape serve --headless` is for agents."
)]
struct Cli {
    ...
    /// Log filter, e.g. `debug` or `runscape_discovery=debug`.
...
enum Command {
    ...
    Capabilities,
    /// Run the local pulse worker: discovery, API, and the visual dashboard
    /// on loopback. Agents should pass `--headless`.
    Serve(ServeArgs),
...
struct ServeArgs {
    ...
    #[arg(long, default_value_t = DEFAULT_PORT, env = "RUNSCAPE_PORT")]
    port: u16,
    ...
    /// SQLite file for history. Defaults to `~/.runscape/runscape.db`.
    #[arg(long)]
    db: Option<PathBuf>,
    ...
    /// stdout, then serve quietly. The visual dashboard is still on `/`; this
    /// flag skips the banner and does not open a browser.
    #[arg(long)]
    headless: bool,

    /// Do not open a browser when starting the visual dashboard.
    #[arg(long)]
    no_open: bool,
}
...
///
/// The bind address is not configurable beyond the port: Runscape serves
...
async fn serve(args: &ServeArgs) -> Result<()> {
    let config = DaemonConfig {
        ...
        snapshot: runscape_server::SnapshotConfig {
        ...
        database: if args.no_persistence {
            ...
            args.db
                .clone()
                .or_else(runscape_server::persistence::default_database_path)
        },
        ..Default::default()
    };
    ...
    let ui = format!("http://localhost:{}", addr.port());
    println!("runscape is up at {ui}");
    match &database {
        Some(path) => println!("  history    {}", path.display()),
        None => println!("  history    in memory only"),
    }
    println!("  dashboard  {ui}");
    println!("  api        http://{addr}/api/v1/status");
    println!("  websocket  ws://{addr}/ws/v1");
    println!(
        "  agents     runscape serve --headless --port {}",
        addr.port()
    );
    if args.no_open {
        println!("open {ui} in a browser");
    } else {
        match open_in_browser(&ui) {
            Ok(()) => println!("opening the dashboard in your browser (--no-open to skip)"),
            Err(error) => println!("open {ui} in a browser ({error})"),
        }
    }
