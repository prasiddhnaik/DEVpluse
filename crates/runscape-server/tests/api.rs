...
use runscape_core::model::{Connection, Evidence, EvidenceType};
use runscape_core::registry::RegistryDelta;
use runscape_core::topology::TopologyDelta;
use runscape_server::api;
use runscape_server::security::OriginPolicy;
use runscape_server::state::{AppState, TickUpdate};
...
async fn projects_summarise_their_services() {
    ...
    assert_eq!(projects[0]["name"], "runscape-fixture");
...
#[tokio::test]
async fn project_detail_sums_resource_history_per_tick() {
    use runscape_core::model::ResourceSample;

    let state = AppState::docker_unknown();
    let web = service("web", 41010, 100);
    let api_svc = service("api", 41011, 200);
    let mut projects = BTreeMap::new();
    let project = project();
    let id = project.id.to_string();
    projects.insert(project.id.clone(), project);
    ...
    first.at = Some(t0);
    first.samples = vec![
        (web.id.clone(), ResourceSample::cpu_and_memory(t0, 1.0, 10)),
        (
            api_svc.id.clone(),
            ResourceSample::cpu_and_memory(t0, 2.0, 30),
        ),
    ];
    state
        .apply_tick(TickUpdate {
            tick: &first,
            projects: &projects,
            services: vec![web.clone(), api_svc.clone()],
            connections: vec![],
            events: Vec::new(),
            warnings: None,
        })
    ...
    let mut second = tick(RegistryDelta::default(), TopologyDelta::default());
    second.at = Some(t1);
    second.samples = vec![
        (web.id.clone(), ResourceSample::cpu_and_memory(t1, 1.5, 11)),
        (
            api_svc.id.clone(),
            ResourceSample::cpu_and_memory(t1, 2.5, 31),
        ),
    ];
    state
        .apply_tick(TickUpdate {
    ...
    let (status, body) = get(&state, &format!("/api/v1/projects/{id}")).await;
    assert_eq!(status, StatusCode::OK);
    let history = body["resource_history"].as_array().expect("history");
    assert_eq!(history.len(), 2);
    assert_eq!(history[0]["cpu_percent"].as_f64().expect("cpu"), 3.0);
    assert_eq!(history[0]["memory_bytes"].as_u64().expect("rss"), 40);
    assert_eq!(history[1]["cpu_percent"].as_f64().expect("cpu"), 4.0);
    assert_eq!(history[1]["memory_bytes"].as_u64().expect("rss"), 42);
}
...
#[tokio::test]
async fn the_embedded_dashboard_origin_is_allowed() {
    let state = seeded_state().await;
    let req = Request::builder()
        .uri("/api/v1/status")
        .header(header::ORIGIN, "http://127.0.0.1:2013")
        .body(Body::empty())
        .expect("request");
...
#[tokio::test]
async fn the_root_serves_the_dashboard_html() {
    let state = seeded_state().await;
    let response = api::router(state, OriginPolicy::default())
        .oneshot(
            Request::builder()
                .uri("/")
                .body(Body::empty())
...
#[tokio::test]
async fn unknown_dashboard_paths_still_serve_the_html_shell() {
    let state = seeded_state().await;
    let response = api::router(state, OriginPolicy::default())
        .oneshot(
            Request::builder()
                .uri("/projects/prj_example")
                .body(Body::empty())
                .expect("request"),
        )
        .await
