...
use runscape_core::model::{EventKind, Project, RunscapeEvent};
...
fn projects() -> BTreeMap<runscape_core::ids::ProjectId, Project> {
    let project = project();
...
async fn publish_stop(state: &AppState) {
    ...
    stopped.health = runscape_core::model::Health::Stopped;
