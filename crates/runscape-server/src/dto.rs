...
use runscape_core::model::{
    Connection, Endpoint, Evidence, Health, HostSample, ProcessInstance, Project, ResourcePeaks,
    ResourceSample, RunscapeEvent, Service, ServiceKind, Warning,
};
...
impl From<&ProcessInstance> for ProcessInstanceDto {
    fn from(p: &ProcessInstance) -> Self {
        Self {
            ...
            virtual_memory_bytes: p.virtual_memory_bytes,
...
impl From<&ResourceSample> for ResourceSampleDto {
    fn from(s: &ResourceSample) -> Self {
        Self {
            ...
            virtual_memory_bytes: s.virtual_memory_bytes,
...
impl From<ResourcePeaks> for ResourcePeaksDto {
    fn from(peaks: ResourcePeaks) -> Self {
        Self {
            cpu_percent: peaks.cpu_percent,
            cpu_at: rfc3339(peaks.cpu_at),
            memory_bytes: peaks.memory_bytes,
            memory_at: rfc3339(peaks.memory_at),
        }
    }
}
...
pub struct ServiceDto {
    ...
    #[serde(default)]
    pub disk_write_bytes: u64,
    pub resources_measured: bool,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub peaks: Option<ResourcePeaksDto>,
...
impl ServiceDto {
    pub fn new(service: &Service, history: &[ResourceSample]) -> Self {
        Self {
            ...
            virtual_memory_bytes: service.total_virtual_memory_bytes(),
            thread_count: service.thread_count(),
            disk_read_bytes: service.total_disk_read_bytes(),
            disk_write_bytes: service.total_disk_write_bytes(),
            resources_measured: service.resources_measured(),
            peaks: ResourceSample::peaks(history).map(ResourcePeaksDto::from),
...
pub struct ProjectSummaryDto {
    ...
    pub ranked_by_cpu: Vec<RankedServiceDto>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub ranked_by_memory: Vec<RankedServiceDto>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub dominant_memory: Option<DominantServiceDto>,
}

/// One service's share of a project's (or `/top`'s) measured CPU or memory.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RankedServiceDto {
    pub id: String,
    pub name: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub project_id: Option<String>,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub peak_cpu_at: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub peak_memory_bytes: Option<u64>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub peak_memory_at: Option<String>,
}

/// Process inside a grouped service — never an ungrouped Chrome helper.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RankedProcessDto {
    pub pid: u32,
    pub parent_pid: Option<u32>,
    pub name: String,
    pub command: Vec<String>,
...
impl ProjectSummaryDto {
    pub fn new(project: &Project, services: &[&Service], recent_warning: Option<&Warning>) -> Self {
        Self {
            ...
            ranked_by_cpu: Vec::new(),
            ranked_by_memory: Vec::new(),
    ...
    fn from(w: &Warning) -> Self {
        Self {
            ...
            severity: match w.severity {
                runscape_core::Severity::Info => "info",
                runscape_core::Severity::Warning => "warning",
                runscape_core::Severity::Critical => "critical",
            }
            .to_string(),
...
impl From<&RunscapeEvent> for EventDto {
    fn from(e: &RunscapeEvent) -> Self {
        Self {
            id: e.id.to_string(),
            at: rfc3339(e.at),
...
impl From<&Service> for GraphNodeDto {
    fn from(s: &Service) -> Self {
        Self {
            ...
            resources_measured: s.resources_measured(),
        }
    }
}
...
pub struct ProjectDetailDto {
    ...
    /// Per-tick sum of this project's service samples. Empty when nothing has
    /// been measured yet. Omitted from the wire when empty.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub resource_history: Vec<ResourceSampleDto>,
...
pub struct RelatedEventDto {
    ...
    /// Never "caused by": Runscape reports adjacency, not causation
    /// (`DECISIONS.md` D008).
    pub relation: String,
...
pub struct DockerStatusDto {
    ...
    /// Per-container CPU/memory. Off unless `runscape serve --docker-stats`.
    #[serde(default)]
    pub stats: bool,
}
...
pub struct StatusDto {
    ...
    pub started_at: String,
    pub uptime_ms: u64,
    /// [`runscape_discovery::platform::PlatformCapabilities`], serialised as-is:
    ...
    /// Latest host sample, absent until the first tick completes.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub host: Option<HostSampleDto>,
    /// Oldest first. Same window as a service resource sparkline.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub host_history: Vec<HostSampleDto>,
}

/// One host-wide sample: load averages and process-table size.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    #[test]
    fn resource_sample_dto_round_trips_the_extra_fields() {
        ...
        let dto = ResourceSampleDto::from(&sample);
        let json = serde_json::to_string(&dto).expect("json");
        let back: ResourceSampleDto = serde_json::from_str(&json).expect("parse");
        assert_eq!(back.cpu_percent, 12.5);
        assert_eq!(back.memory_bytes, 4096);
        assert_eq!(back.virtual_memory_bytes, 8192);
        assert_eq!(back.thread_count, 7);
        assert_eq!(back.disk_read_bytes, 128);
        assert_eq!(back.disk_write_bytes, 64);
        assert_eq!(back.connection_count, 2);
        assert_eq!(back.at, "1970-01-01T00:00:00Z");
    }

    #[test]
    fn host_sample_dto_round_trips() {
        ...
        let dto = HostSampleDto::from(&sample);
        let json = serde_json::to_string(&dto).expect("json");
        let back: HostSampleDto = serde_json::from_str(&json).expect("parse");
        assert_eq!(back.load_avg_1, 1.25);
        assert_eq!(back.process_count, 412);
        assert_eq!(back.at, "1970-01-01T00:00:00Z");
    }

    #[test]
    fn old_resource_sample_json_fills_connection_count() {
        let dto: ResourceSampleDto = serde_json::from_str(
            r#"{"at":"1970-01-01T00:00:00Z","cpu_percent":1.0,"memory_bytes":10}"#,
        )
        .expect("legacy");
        assert_eq!(dto.connection_count, 0);
        assert_eq!(dto.virtual_memory_bytes, 0);
        assert_eq!(dto.thread_count, 0);
    }
}
