...
use runscape_core::ids::{ConnectionId, ProjectId, ServiceId};
use runscape_core::model::{Connection, Project, ResourceSample, RunscapeEvent, Service, Warning};
use runscape_core::resources::{HostHistory, ResourceHistory, sum_aligned};
use runscape_discovery::platform::{self, PlatformCapabilities};
use tokio::sync::{RwLock, RwLockReadGuard, broadcast};
use tracing::warn;

use crate::dto::{
    CollectorStatusDto, CollectorsDto, ConnectionDto, CountsDto, DockerStatusDto, HostSampleDto,
...
use runscape_events::warnings::{WarningEngine, WarningInput};
...
pub struct RuntimeView {
    ...
    /// Newest last.
    pub events: VecDeque<RunscapeEvent>,
...
impl RuntimeView {
    ...
    pub fn recent_events(&self, filter: &EventFilter) -> Vec<&RunscapeEvent> {
        self.events
            .iter()
            .rev()
    ...
    pub fn project_summary(&self, project: &Project) -> ProjectSummaryDto {
        ...
        let rankings = crate::attribution::rank_running_services(
            &services,
            |_| Some(project.name.as_str()),
            |id| self.resources.history(id),
            None,
        );
        summary.ranked_by_cpu = rankings.by_cpu;
        summary.ranked_by_memory = rankings.by_memory;
        summary.dominant_memory = rankings.dominant_memory;
        summary
    }

    /// CPU and RSS of every service in the project, summed per tick.
    pub fn project_resource_history(&self, project: &ProjectId) -> Vec<ResourceSample> {
        sum_aligned(
            self.services_of(project)
                .map(|service| self.resources.history(&service.id)),
        )
    }
    ...
    fn push_event(&mut self, event: RunscapeEvent) {
...
impl EventFilter {
    ...
    fn matches(&self, event: &RunscapeEvent, services: &BTreeMap<ServiceId, Service>) -> bool {
...
pub fn event_service(event: &RunscapeEvent) -> Option<&ServiceId> {
    ...
    match &event.kind {
        ...
        // A port can open before Runscape knows whose it is.
...
impl AppState {
    ...
    pub fn status_of(&self, view: &RuntimeView) -> StatusDto {
        StatusDto {
            ...
            host: view.host.latest().as_ref().map(HostSampleDto::from),
            host_history: view
                .host
                .history()
                .iter()
                .map(HostSampleDto::from)
                .collect(),
        }
    }
    ...
    pub async fn apply_tick(&self, update: TickUpdate<'_>) -> AppliedTick {
        ...
        // Runscape cannot attribute, and it is still reported.
        ...
        if let Some(engine) = warnings {
            let history: BTreeMap<ServiceId, Vec<runscape_core::model::ResourceSample>> = view
                .services
                .keys()
                .map(|id| (id.clone(), view.resources.history(id)))
                .collect();
            let window = engine.rules().event_window;
            let recent: Vec<RunscapeEvent> = view
                .events
                .iter()
                .rev()
...
fn changed_services(delta: &runscape_core::registry::RegistryDelta) -> Vec<ServiceId> {
...
mod tests {
    ...
    fn started(service: &str, secs: u64) -> RunscapeEvent {
        RunscapeEvent {
