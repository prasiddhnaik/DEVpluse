...
use runscape_core::grouping::{GroupingEngine, GroupingInput};
use runscape_core::identity::{Runtime, ServiceFingerprint};
use runscape_core::ids::{ProjectId, ServiceId};
use runscape_core::model::{Endpoint, ProcessInstance, Protocol, Service, ServiceKind};
use runscape_core::model::{HostSample, Project, ResourceSample};
use runscape_core::project::{ProjectResolver, ResolverConfig};
use runscape_core::registry::{RegistryDelta, ServiceObservation, ServiceRegistry};
use runscape_core::service_filter::{is_build_tool, is_bundled_app, is_system_tool};
use runscape_core::topology::{ObservedConnectionEndpoints, TopologyBuilder, TopologyDelta};
use runscape_discovery::error::CollectorError;
use runscape_discovery::process::{ObservedProcess, ProcessCollector, SysinfoProcessCollector};
use runscape_discovery::socket::{Netstat2SocketCollector, SocketCollector};
...
pub struct TickResult {
    ...
    /// Host load and process-table size from this tick's process snapshot.
...
impl SnapshotLoop {
    ...
    pub async fn tick(&mut self) -> Result<TickResult, SnapshotError> {
        ...
        let mut process_snapshot =
            process_snapshot.map_err(|e| SnapshotError::CollectionFailed {
        ...
        // Disk I/O and thread counts are collected only for processes that
        // grouping will keep — not every Chrome helper on the machine.
        if let Err(error) = self
            .process_collector
            .enrich(&mut process_snapshot, &group_pids)
            .await
        {
            warn!(%error, "process extra sampling failed; continuing with CPU and memory");
        }
        ...
        for process in &process_snapshot.processes {
            ...
            let endpoints: Vec<Endpoint> = socket_snapshot
                ...
                .map(|s| Endpoint {
                    ...
                    protocol: match s.protocol {
                        runscape_discovery::socket::Protocol::Tcp => Protocol::Tcp,
                        runscape_discovery::socket::Protocol::Udp => Protocol::Udp,
                    },
            ...
            // loop (`service_filter`).
            if !runscape_core::service_filter::is_service_process(
                process.executable.as_deref(),
                !endpoints.is_empty(),
        ...
        let topology = self.topology_builder.topology();
        ...
        let samples: Vec<(ServiceId, ResourceSample)> = services
            .iter()
            .filter(|s| s.is_running() && s.resources_measured())
            .map(|s| {
                (
                    s.id.clone(),
                    ResourceSample {
                        ...
                        virtual_memory_bytes: s.total_virtual_memory_bytes(),
                        thread_count: s.total_thread_count(),
                        disk_read_bytes: s.total_disk_read_bytes(),
                        disk_write_bytes: s.total_disk_write_bytes(),
                        connection_count: u32::try_from(topology.touching(&s.id).count())
                            .unwrap_or(u32::MAX),
                    },
                )
            })
        ...
        Ok(TickResult {
            ...
            host: Some(HostSample {
                at,
                load_avg_1: process_snapshot.load_avg_1,
                load_avg_5: process_snapshot.load_avg_5,
                load_avg_15: process_snapshot.load_avg_15,
    ...
    pub fn topology(&self) -> &runscape_core::topology::Topology {
...
fn degraded_fields(d: &runscape_discovery::process::Degradations) -> BTreeMap<String, usize> {
...
mod tests {
    ...
    fn shells_and_compilers_are_not_grouped() {
        let shell = ObservedProcess {
            ...
            run_time_secs: 600,
            state: runscape_discovery::process::ProcessState::Sleeping,
            user_id: None,
        };
