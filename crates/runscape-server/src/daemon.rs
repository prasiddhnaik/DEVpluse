...
use runscape_core::ids::ProjectId;
use runscape_discovery::watcher::ProjectWatcher;
use runscape_docker::availability::DockerAvailability;
use runscape_docker::collector::{BollardCollector, ContainerCollector};
use runscape_events::EventDeriver;
use runscape_events::warnings::{WarningEngine, WarningRules};
...
use runscape_storage::RetentionPolicy;
...
impl Daemon {
    pub async fn bind(config: DaemonConfig) -> anyhow::Result<Self> {
        if !is_loopback_bind(&config.bind) {
            ...
            // (`AGENTS.md` rule 6).
            bail!(
                "refusing to bind {}: Runscape serves loopback only",
                config.bind
            );
        }
        ...
        let origin_policy = config.origin_policy.with_bind(addr);
        ...
        let router = api::router(state.clone(), origin_policy);
    ...
    pub async fn serve_until(
        ...
        info!(%addr, "runscape daemon listening");
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Some(watcher) = &mut watcher {
                ...
                // way Runscape can correlate with anything.
...
fn changed_service_ids(
    delta: &runscape_core::registry::RegistryDelta,
) -> std::collections::BTreeSet<runscape_core::ids::ServiceId> {
    delta
        .started
        .iter()
        .map(|(id, _)| id.clone())
