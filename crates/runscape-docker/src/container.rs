...
//! what Runscape models. No I/O happens here, which is why the mapping rules
...
/// (`removing`, `stopping`, …). Anything Runscape does not recognise becomes
...
pub struct ContainerPort {
    ...
    /// host, so it is not an endpoint Runscape can correlate to a host socket.
...
impl ContainerPort {
    ...
    ///
    /// Returns `None` for protocols Runscape has no domain type for (SCTP):
...
impl ObservedContainer {
    ...
    /// container must not blind Runscape to the rest.
    ...
    /// [`ProcessInstance`](runscape_core::ProcessInstance) to report. For a
    ...
    pub fn to_service(&self, project_id: Option<ProjectId>, at: SystemTime) -> Service {
        ...
        Service {
            ...
            // know about restarts.
            restart_count: 0,
            measured_cpu_percent: self.cpu_percent,
            measured_memory_bytes: self.memory_bytes,
        }
    }
}
...
mod tests {
    ...
    fn to_service_prefers_the_compose_service_name() {
        ...
        assert!(
            service.measured_cpu_percent.is_none() && service.measured_memory_bytes.is_none(),
            "stats off is unmeasured, not zero idle"
        );
        assert_eq!(
            service.id,
    ...
    #[test]
    fn to_observation_carries_docker_stats_when_present() {
        ...
        let observation = observed.to_observation(None);
        assert_eq!(observation.measured_cpu_percent, Some(12.5));
        assert_eq!(observation.measured_memory_bytes, Some(64 * 1024 * 1024));
        assert!(observation.instance.is_none());
    }
